...
public class FiszkiMatWspolFragment extends Fragment {
    ...
    public void onStart () {
        ...
        if (liczba == 1) {textViewDwa.setText("Praca wyjścia wyrażona długością fali, natężeniem");
            String Fiszka2 = "'$W_0=hν_{gra.}={hc}/λ_{gra.}$'";
            Fiszka = Fiszka2;}
        else if (liczba == 2) {textViewDwa.setText("Energia kinetyczna maksymalna");
            String Fiszka3 = "'$E_k_{max}={mv^2}/2=eU_h$'";
            Fiszka = Fiszka3;}
        else if (liczba == 3) {textViewDwa.setText("Rówannie orbity atomu modelu Bohra");
            String Fiszka4 = "'$mV_nr_n=n{h}/{2π}$'";
            Fiszka = Fiszka4;}
        else if (liczba == 4) {textViewDwa.setText("Stosunek prędkości elektronów na orbitach modelu Bohra");
            String Fiszka5 = "'$v_1:v_2:{v_3}...{v_n}=1:1/2:{1/3}....{1/n}$'";
            Fiszka = Fiszka5;}
        else if (liczba == 5) {textViewDwa.setText("Stosunek energii elektronów na orbitach modelu Bohra");
            String Fiszka6 = "'$E_1:E_2.....E_n=1:{1/2^2}....{1/n^2}$'";
            Fiszka = Fiszka6;}
        else if (liczba == 6) {textViewDwa.setText("Energia jonizacji");
            String Fiszka8 = "'$E_j=E_∞-E_1=13,6[eV]$'";
            Fiszka = Fiszka8;}
        else if (liczba == 7) {textViewDwa.setText("Częstotliwość widma");
            String Fiszka9 = "'$ν=cR(1/n^2-1/k^2)$'";
            Fiszka = Fiszka9;}
        else if (liczba == 8) {textViewDwa.setText("Deficity masy");
            String Fiszka10 = "'$Δm=m_{sk}-m_{j}=Zm_p+(A-Z)m_n-m_j$'";
            Fiszka = Fiszka10;}
        else if (liczba == 9) {textViewDwa.setText("Trwałośc wiązania");
            String Fiszka11 = "'${ΔE/A} $'";
            Fiszka = Fiszka11;}
        else if (liczba == 10) {textViewDwa.setText("Promieniowanie alfa");
            String Fiszka11 = "'$^A_ZX->_{Z+1}^{A}Y+_{-1}^0β^{-}$'";
            Fiszka = Fiszka11;}
        else if (liczba == 11) {textViewDwa.setText("Promieniowanie beta-minus");
            String Fiszka11 = "'$^A_ZX->_{Z+1}^{A}Y+_{-1}^0β^{-}$'";
            Fiszka = Fiszka11;}
        else if (liczba == 12) {textViewDwa.setText("Promieniowanie beta-plus");
            String Fiszka11 = "'$^A_ZX->_{Z+1}^{A}Y+_{1}^0β^{+}$'";
            Fiszka = Fiszka11;}
        else if (liczba == 13) {textViewDwa.setText("Zależność masy początkowej od czasu");
            String Fiszka11 = "'$N(t)=N_0-(1/2)^{t/T_{1/2}}$'";
            Fiszka = Fiszka11;}
        else if (liczba == 14) {textViewDwa.setText("Zasada nieznaczoności (pęd)");
            String Fiszka11 = "'$ΔxΔp≥h/{4π}$'";
            Fiszka = Fiszka11;}
        else if (liczba == 15) {textViewDwa.setText("Zasada nieoznaczoności (Energia)");
